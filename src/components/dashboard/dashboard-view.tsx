    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          h1 { color: #db2777; font-size: 16pt; margin-bottom: 5px; }
          h2 { color: #be185d; font-size: 12pt; margin-top: 0; }
          table { width: 100%; border-collapse: collapse; margin-top: 15px; }
          th { background: #db2777; color: white; padding: 6px 4px; text-align: left; font-size: 9pt; }
          ...
          .soldes { margin-top: 15px; padding: 10px; background: #fce7f3; border-radius: 6px; }
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Bienvenue sur SRGA V1.99</p>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-pink-100 hover:bg-pink-100">
              <TableHead className="font-bold text-pink-700">Libellé</TableHead>
              {[6, 5, 4, 3, 2, 1, 0].map((m) => (
                <TableHead key={m} className={`text-right font-bold text-pink-700 ${m === 0 ? 'bg-pink-200' : ''}`}>
                  {getMonthLabel(m)}
                </TableHead>
              ))}
            </TableRow>
          ...
          <TableBody>
            {tableRows.map((row, idx) => (
              <TableRow key={idx} className="hover:bg-pink-100/50">
                <TableCell className="font-medium text-pink-800">{row.label}</TableCell>
                {[6, 5, 4, 3, 2, 1, 0].map((m) => {
                  const key = `m${m}` as keyof MonthlyData;
                  return (
                    <TableCell key={m} className={`text-right font-mono ${m === 0 ? 'bg-pink-100 font-bold' : ''}`}>
