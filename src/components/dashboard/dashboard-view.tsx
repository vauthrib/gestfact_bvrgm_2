...
export function DashboardView() {
  ...
  const MonthlyStatBar = ({ data, label }: { data: MonthlyData, label: string }) => (
    <div className="bg-pink-50 rounded-lg p-2 mb-2 border border-pink-200">
      <div className="text-xs text-pink-600 font-medium mb-1">{label}</div>
      <div className="grid grid-cols-7 gap-1 text-center">
        {[6, 5, 4, 3, 2, 1, 0].map((m) => {
          const key = `m${m}` as keyof MonthlyData;
          const isCurrent = m === 0;
          return (
            <div key={m} className={isCurrent ? 'bg-pink-100 rounded px-1' : ''}>
              <div className="text-[9px] text-muted-foreground font-medium">{getMonthLabel(m)}</div>
              <div className={`text-[10px] font-bold ${isCurrent ? 'text-pink-800' : 'text-pink-700'}`}>
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Bienvenue sur SRGA V1.98</p>
        </div>
