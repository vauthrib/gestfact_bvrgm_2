    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Bienvenue sur SRGA V2.00</p>
        </div>
          ...
          <TableBody>
            {tableRows.map((row, idx) => (
              <TableRow key={idx} className="hover:bg-pink-100/50">
                <TableCell className="font-medium text-pink-800 border-r border-pink-200">{row.label}</TableCell>
                {[0, 1, 2, 3, 4, 5, 6].map((m) => {
