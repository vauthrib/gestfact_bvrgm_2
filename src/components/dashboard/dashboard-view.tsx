    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Bienvenue sur SRGA V2.71</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="bg-pink-100 text-pink-700 px-3 py-1 rounded-full text-sm font-mono font-bold">TDB01</span>
          <Button variant="outline" className="border-pink-600 text-pink-700" onClick={openEtatClientDialog}>
            <FileText className="w-4 h-4 mr-2" />Etat Client
          </Button>
          <Button variant="outline" className="border-pink-600 text-pink-700" onClick={openRelanceClientDialog}>
            <AlertCircle className="w-4 h-4 mr-2" />Relance client
          </Button>
          <Button className="bg-pink-600 hover:bg-pink-700" onClick={openReleveDialog}>
          ...
          <TableBody>
            {tableRows.map((row, idx) => (
              <TableRow key={idx} className="hover:bg-pink-100/50">
                <TableCell className={`font-medium text-green-800 border-r border-pink-200 ${row.isTTC ? 'text-xs' : ''}`}>{row.label}</TableCell>
                {[0, 1, 2, 3, 4, 5, 6].map((m) => {
                  const key = `m${m}` as keyof MonthlyData;
                  return (
                    <TableCell key={m} className={`text-right font-mono ${m === 0 ? 'bg-pink-100 font-bold' : ''} ${row.isTTC ? 'text-xs' : ''}`}>
      ...
      <Dialog open={releveDialogOpen} onOpenChange={setReleveDialogOpen}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button className="bg-pink-600 hover:bg-pink-700" onClick={generateReleve}>Générer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
              ...
              <>
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    <TableRow className="bg-pink-50 font-bold"><TableCell colSpan={4}>TOTAL</TableCell><TableCell></TableCell><TableCell className="text-right">{formatCurrency(Math.abs(releveData.reduce((s, l) => s + l.montant, 0)))}</TableCell><TableCell></TableCell></TableRow>
                  </TableBody>
                </Table>
              </>
            )}
      ...
      <Dialog open={etatClientDialogOpen} onOpenChange={setEtatClientDialogOpen}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button className="bg-pink-600 hover:bg-pink-700" onClick={generateEtatClient}>Générer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
                ...
                <Table>
                  ...
                  <TableBody>
                    {etatClientData.map((line, idx) => {
                      let bgClass = '';
                      let typeLabel = '';
                      if (line.type === 'facture') { bgClass = 'bg-pink-50'; typeLabel = 'Facture'; }
                      else if (line.type === 'avoir') { bgClass = 'bg-yellow-50'; typeLabel = '  └ Avoir'; }
                      else if (line.type === 'reglement') { bgClass = 'bg-pink-50'; typeLabel = '  └ Règlement'; }
