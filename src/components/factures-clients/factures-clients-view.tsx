            ...
            <Table>
              ...
              <TableBody>{filtered.map((f) => (<TableRow key={f.id}>
                ...
                <TableCell><div className="flex gap-1 flex-wrap">
                  ...
                  {f.statut === 'VALIDEE' && (
                    <Button size="sm" variant="outline" className="text-pink-700" onClick={() => handleView(f)} title="Visualiser la facture"><Eye className="h-4 w-4" /></Button>
                  )}
                  {/* V2.93 - Visualiser détaillé (BL liés) si facture groupée */}
                  {f.statut === 'VALIDEE' && f.numeroBL && (
                    <Button size="sm" variant="outline" className="text-pink-600" onClick={() => handleViewDetailed(f)} title="Visualiser détaillé (BL liés)"><EyeOff className="h-4 w-4" /></Button>
                  )}
                  </PermissionGate>
                  <Button size="sm" variant="outline" onClick={() => handlePrint(f)} title="Imprimer"><Printer className="h-4 w-4" /></Button>
                  <PermissionGate permission="scans.view"><Button size="sm" variant="outline" className="text-pink-600 hover:text-pink-800" onClick={() => { setScanDocNumero(f.numero); setScanDialogOpen(true); }} title="Scans & Accusé Réception (AR)"><Paperclip className="h-4 w-4" /></Button></PermissionGate>
      ...
      <Dialog open={multiArticleDialogOpen} onOpenChange={setMultiArticleDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <DialogFooter>
            ...
            <Button className="bg-pink-600 hover:bg-pink-700" onClick={handleAddMultipleArticles} disabled={selectedArticles.length === 0}>
        ...
        <DialogContent className="max-w-3xl max-h-[calc(100vh-4rem)] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              ...
              <span className="bg-pink-100 text-pink-700 px-3 py-1 rounded-full text-sm font-mono font-bold">NFC01-VISU</span>
            </div>
          </DialogHeader>
          {viewingFacture && (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div><Label className="text-muted-foreground">N° Facture</Label><div className="font-bold text-pink-700">{viewingFacture.numero}</div></div>
                ...
                <div><Label className="text-muted-foreground">Statut</Label><div><span className={`px-2 py-1 rounded text-xs ${viewingFacture.statut === 'VALIDEE' ? 'bg-pink-100 text-pink-800' : 'bg-yellow-100 text-yellow-800'}`}>{viewingFacture.statut === 'VALIDEE' ? 'Validée' : 'Brouillon'}</span></div></div>
              </div>
              <div className="grid grid-cols-3 gap-4">
        ...
        <DialogContent className="max-w-6xl max-h-[calc(100vh-4rem)] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              ...
              <span className="bg-pink-100 text-pink-700 px-3 py-1 rounded-full text-sm font-mono font-bold">NFC01-DETAILED</span>
            </div>
          </DialogHeader>
          {viewingFacture && (
            <div className="space-y-5 py-2">
              ...
              <div className="bg-gray-50 border rounded-lg p-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 text-sm">
                  <div><span className="text-muted-foreground text-xs">N° Facture</span><div className="font-bold text-pink-700">{viewingFacture.numero}</div></div>
                  ...
                  <div><span className="text-muted-foreground text-xs">Total TTC</span><div className="font-bold text-pink-700">{formatCurrency(viewingFacture.totalTTC)}</div></div>
                </div>
              </div>
              {/* Recap par référence article × BL */}
              {viewingBLs.length > 0 && (
                <div className="border rounded-lg">
                  <div className="bg-pink-50 px-4 py-2 border-b flex items-center justify-between">
                    <Label className="font-semibold text-pink-700">Récapitulatif par référence ({buildRecapTable().length} article{buildRecapTable().length > 1 ? 's' : ''})</Label>
                    <span className="text-xs text-pink-600">{viewingBLs.length} BL</span>
                  </div>
                      ...
                      <TableBody>
                        {buildRecapTable().map((r: any, idx: number) => (
                          <TableRow key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                            ...
                            <TableCell className="text-right font-bold text-pink-700">{r.totalQte}</TableCell>
                            {viewingBLs.map((bl) => (
                    ...
                    {buildRecapTable().map((r: any, idx: number) => (
                      <div key={idx} className="p-3 space-y-2">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="shrink-0 text-right"><div className="text-[10px] text-muted-foreground">Qté totale</div><div className="font-bold text-pink-700">{r.totalQte}</div></div>
                        </div>
              ...
              {viewingBLs.map((bl, blIdx) => {
                const blTotal = (bl.lignes || []).reduce((s: number, l: any) => s + (l.totalHT || 0), 0);
                return (
                <div key={bl.id} className="border rounded-lg">                    <div className="bg-pink-50 px-4 py-2 border-b border-pink-200 flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-3">
                      <span className="bg-pink-100 text-pink-700 text-xs font-mono font-bold px-2 py-0.5 rounded">BL {blIdx + 1}/{viewingBLs.length}</span>
                      <Label className="font-semibold break-words whitespace-normal">{bl.numero}</Label>
                    </div>
                    <span className="text-sm text-muted-foreground">{new Date(bl.dateBL).toLocaleDateString('fr-FR')} — <span className="font-medium text-pink-700">{formatCurrency(bl.totalHT || blTotal)}</span></span>
                  </div>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button variant="outline" className="text-pink-600" onClick={printRecap}>
              <Printer className="h-4 w-4 mr-1" />Imprimer Récap
            </Button>
            <Button variant="outline" className="text-pink-600" onClick={printDetailed}>
              <Printer className="h-4 w-4 mr-1" />Imprimer Détails BL
            </Button>
            <Button className="bg-pink-600 hover:bg-pink-700" onClick={() => { setViewDialogOpen(false); setViewDetailed(false); if (viewingFacture) handlePrint(viewingFacture); }}>
