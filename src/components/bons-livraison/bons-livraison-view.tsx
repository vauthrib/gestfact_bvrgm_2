          ...
          {filtered.length === 0 ? <div className="text-center text-muted-foreground py-8">Aucun BL</div> : (
            <Table>
              ...
              <TableBody>{filtered.map((b) => (<TableRow key={b.id} className={selectedBLs.includes(b.id) ? 'bg-pink-50' : ''}>
                ...
                <TableCell>
                  {b.facture ? (
                    <span className={`px-2 py-1 rounded text-xs font-medium ${getBlsModifies(b.facture.id).length > 0 ? 'bg-orange-100 text-orange-800' : 'bg-pink-100 text-pink-800'}`} title={getBlsModifies(b.facture.id).length > 0 ? 'BL modifié(s) après facturation - mise à jour de la facture requise' : ''}>
                ...
                <TableCell><div className="flex gap-1 flex-wrap">
                  ...
                  {b.statut === 'VALIDEE' && (
                    <PermissionGate permission="factures.create">
                      <Button 
                        size="sm" 
                        variant="outline" 
                        className={b.facture ? "text-gray-400 cursor-not-allowed" : "text-pink-600"} 
                  ...
                  {b.statut === 'VALIDEE' && (
                    <Button size="sm" variant="outline" className="text-pink-700" onClick={() => handleViewBL(b)} title="Visualiser le BL">
                  ...
                  <Button size="sm" variant="outline" className="text-pink-700" onClick={() => setArchiveBL(b)} title="Archiver l’expédition"><Archive className="h-4 w-4" /></Button>
        ...
        <DialogContent className="max-w-3xl max-h-[calc(100vh-4rem)] overflow-y-auto">
          <DialogHeader>
            <div className="flex items-center justify-between">
              ...
              <span className="bg-pink-100 text-pink-700 px-3 py-1 rounded-full text-sm font-mono font-bold">NBL01-VUE</span>
            </div>
          </DialogHeader>
          {viewingBL && (
            <div className="space-y-4 py-2">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  ...
                  <div className="font-bold text-pink-700">{viewingBL.numero}</div>
                </div>
                <div>
              ...
              <div className="flex items-center justify-between border-t pt-3">
                <span className={`px-2 py-1 rounded text-xs ${viewingBL.statut === 'VALIDEE' ? 'bg-pink-100 text-pink-800' : 'bg-yellow-100 text-yellow-800'}`}>{viewingBL.statut === 'VALIDEE' ? 'Validé' : 'Brouillon'}</span>
