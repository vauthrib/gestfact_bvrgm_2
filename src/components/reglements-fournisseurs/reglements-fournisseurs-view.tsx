    ...
    <div className="p-6 space-y-6 w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <PermissionGate permission="reglements-fourn.create"><Button className="bg-pink-600 hover:bg-pink-700" onClick={() => { resetForm(); setDialogOpen(true); }}><Plus className="w-4 h-4 mr-2" />Nouveau</Button></PermissionGate>
        </div>
      </div>
      <Card>
              ...
              <TableBody>
                {groupedData.map((item, idx) => {
                  if (isGroupedReglement(item)) {
                    ...
                    return (
                      <>
                        <TableRow key={item.baseNumber} className="bg-pink-50 font-semibold">
                          <TableCell>
                          ...
                          <TableCell className="font-bold text-pink-700">{item.fournisseur}</TableCell>
                          <TableCell className="text-pink-600 italic">{item.reglements.length} facture(s)</TableCell>
                          <TableCell className="font-bold text-pink-700">{formatCurrency(item.totalMontant)}</TableCell>
                          <TableCell>{getModePaiementDisplay(item.modePaiement, null)}</TableCell>
                          <TableCell>
                            <span className={`px-2 py-1 rounded text-xs ${item.statut === 'VALIDE' ? 'bg-pink-100 text-pink-800' : 'bg-yellow-100 text-yellow-800'}`}>
                ...
                {multiPayments.length === 0 ? (
                  ...
                  <>
                      ...
                      <TableBody>
                        {multiPayments.map((p) => (
                          <TableRow key={p.factureId}>
                            ...
                            <TableCell className="text-pink-600 font-semibold">{formatCurrency(p.resteAPayer)}</TableCell>
                            <TableCell>
                    ...
                    <div className="mt-4 p-3 bg-white rounded border flex justify-between items-center">
                      ...
                      <span className="text-xl font-bold text-pink-600">{formatCurrency(calculateTotalMultiPayment())}</span>
                    </div>
                  </>
                )}
              </div>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {resteAPayer !== null && parseNumber(formData.montant) > resteAPayer && <div className="flex items-center gap-2 mt-1 text-pink-600 text-sm"><AlertTriangle className="w-4 h-4" />Dépassement de {formatCurrency(parseNumber(formData.montant) - resteAPayer)}</div>}
                  </div>
                </div>
              </>
            )}
